...
public class Player : MonoBehaviour, I_HasProgress {
    ...
    private void Start() {
        ...
        experienceManager.OnLevelUp += ExperienceManager_OnLevelUp;
    ...
    private void OnTriggerEnter(Collider other) {
        ...
        if (other.CompareTag("Pickup")) {
            if (experienceManager != null) {
                experienceManager.GainExperience(1);
            } else {
                Debug.LogWarning("ExperienceManager is not assigned.");
            }
            Destroy(other.gameObject);
        }
        if (other.CompareTag("PickupLife")) {
            Heal(100);
            Destroy(other.gameObject);
        }
    }
