...
public class Enemy : MonoBehaviour, I_HasProgress {
    ...
    private const string DEAD_ENEMY = "DeadEnemy";
    ...
    private void Death() {
        ...
        gameObject.layer = LayerMask.NameToLayer(DEAD_ENEMY);
        ...
        if (randomNumber < 9) {
            Instantiate(exp, new Vector3(transform.position.x, 0, transform.position.z), Quaternion.identity);
        } else {
            Instantiate(life, new Vector3(transform.position.x, 0, transform.position.z), Quaternion.identity);
        }
